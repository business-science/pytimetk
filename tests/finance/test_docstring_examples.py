...
def test_docstring_adx_examples(stocks_subset):
    ...
    assert {"close_adx_14", "close_plus_di_14", "close_minus_di_14"}.issubset(pandas_result.columns)
    assert {"close_adx_28", "close_plus_di_28", "close_minus_di_28"}.issubset(pandas_result.columns)

    polars_result = (
        pl.from_pandas(stocks_subset.query("symbol == 'AAPL'"))
        .tk.augment_adx(
            date_column="date",
            high_column="high",
            low_column="low",
    ...
    assert {"close_adx_14", "close_plus_di_14", "close_minus_di_14"}.issubset(polars_result.columns)
...
def test_docstring_drawdown_examples(stocks_subset):
    ...
    assert {"close_peak", "close_drawdown", "close_drawdown_pct"}.issubset(pandas_result.columns)
    ...
    assert {"close_peak", "close_drawdown", "close_drawdown_pct"}.issubset(polars_result.columns)
...
def test_docstring_ewma_volatility_examples(stocks_subset):
    ...
    assert {"close_ewma_vol_20_0.94", "close_ewma_vol_50_0.94"}.issubset(example_one.columns)
    ...
    assert {"close_ewma_vol_20_0.94", "close_ewma_vol_50_0.94"}.issubset(example_two.columns)

    example_three = (
        pl.from_pandas(single_stock)
...
def test_docstring_fip_momentum_examples(stocks_subset):
    ...
    assert {"close_fip_momentum_63", "close_fip_momentum_252"}.issubset(polars_result.columns)
...
def test_docstring_regime_detection_examples(stocks_subset):
    pytest.importorskip("hmmlearn.hmm", reason="Regime detection example requires hmmlearn")
