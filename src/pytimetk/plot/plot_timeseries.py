...
def _plot_timeseries_plotly(
    ...
    if plotly_dropdown and group_names is not None:
        ...
        if group_names is not None:
            ...
            group_lookup_df["_group_names"] = group_lookup_df[group_names].agg(
                " | ".join, axis=1
            )
            ...
            subplot_titles = [
                " | ".join(map(str, name)) if isinstance(name, tuple) else str(name)
...
def _plot_timeseries_plotnine(
    ...
    if color_column is not None:
        ...
        data["_color_group_names"] = pd.Categorical(
            data["_color_group_names"],
            categories=data["_color_group_names"].unique(),
            ordered=True,
        )
    ...
    if group_names is not None:
        ...
        data["_group_names"] = pd.Categorical(
            data["_group_names"], categories=data["_group_names"].unique(), ordered=True
        )
