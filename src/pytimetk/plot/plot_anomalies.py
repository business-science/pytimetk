    ...
    if engine == "plotly":
        ...
        if engine == "matplotlib":
            ...
            if height == None:
                ...
                height_size = height
            fig = fig + theme_timetk(
                height=height_size, width=width_size
            )  # setting default figure size to prevent matplotlib sizing error
            fig = fig.draw()
...
def _plot_anomalies_plotly(
    ...
    preload_plot_timeseries = partial(
        ...
        value_column="_val",
        color_column="_var",
        color_palette=[line_color, ribbon_color_hex, ribbon_color_hex],
        facet_ncol=facet_ncol,
        facet_nrow=facet_nrow,
        facet_scales=facet_scales,
        facet_dir=facet_dir,
    ...
    data_prepared = data[
        [*group_names, date_column, "observed", "recomposed_l1", "recomposed_l2"]
    ].melt(
    ...
    if plotly_dropdown and is_grouped:
        ...
        for idx, (group_name_values, group_data) in enumerate(
            data_prepared.groupby(group_names)
        ):
            ...
            if observed_trace and recomposed_l1_trace and recomposed_l2_trace:
                ...
                for x, y, y_l1, y_l2 in zip(
                    observed_trace.x,
                    observed_trace.y,
                    recomposed_l1_trace.y,
                    recomposed_l2_trace.y,
                ):
                    if y > y_l2 or y < y_l1:
                        x_values.append(x)
                        y_values.append(y)
        ...
        indices_l1 = [
            i for i, trace in enumerate(fig.data) if "recomposed_l1" in trace.name
        ]
        indices_l2 = [
            i for i, trace in enumerate(fig.data) if "recomposed_l2" in trace.name
        ]
        ...
        for indices in traces_per_subplot:
            ...
            if observed_data and recomposed_l2_data and recomposed_l1_data:
                ...
                for x, y, y_l1, y_l2 in zip(
                    observed_data.x,
                    observed_data.y,
                    recomposed_l1_data.y,
                    recomposed_l2_data.y,
                ):
                    if y > y_l2 or y < y_l1:
                        x_values.append(x)
                        y_values.append(y)
...
def _plot_anomalies_plotnine(
    ...
    # 4.0 Add Lines
    g = g + geom_line(
        color=line_color, size=line_size, linetype=line_type, alpha=line_alpha
    )
    ...
    if y_intercept is not None:
        g = g + geom_hline(yintercept=y_intercept, color=y_intercept_color)

    # Add a X-Intercept if desired
    if x_intercept is not None:
        g = g + geom_vline(xintercept=x_intercept, color=x_intercept_color)
    ...
    # Add theme
    g = g + theme_timetk(base_size=base_size, width=width, height=height)
