...
def _patch_groupby_obj_access() -> None:
    ...
    def _getter(self):
        if hasattr(self, "__dict__") and "obj" in self.__dict__:
            ...
            if hasattr(candidate, "to_pandas") and not isinstance(
        ...
        if (
            ...
            and hasattr(candidate, "to_pandas")
            and not isinstance(candidate, pd.DataFrame)
        ):
            try:
                candidate = candidate.to_pandas()
            except Exception:
...
def convert_to_engine(
    ...
    if engine == "pandas":
        if original_kind in ("pandas_df", "pandas_groupby"):
            group_cols = (
                _extract_pandas_group_columns(data)
...
def restore_output_type(
    ...
    if conversion.row_id_column:
        if has_polars():
            ...
            if (
                ...
                and conversion.row_id_column in result.columns
            ):
                result = result.sort(conversion.row_id_column).drop(
        ...
        if (
            ...
            result = result.sort_values(conversion.row_id_column).drop(
                columns=[conversion.row_id_column]
            )
        elif (
            cudf is not None
            and isinstance(result, cudf.DataFrame)
            and conversion.row_id_column in result.columns
        ):
            result = result.sort_values(conversion.row_id_column).drop(
                columns=[conversion.row_id_column]
    ...
    if conversion.original_kind in ("pandas_df", "pandas_groupby"):
        ...
        if cudf is not None and isinstance(result, cudf.DataFrame):
            result = result.to_pandas()
        if conversion.pandas_index is not None and len(conversion.pandas_index) == len(
            result
        ):
    ...
    if conversion.original_kind in ("cudf_df", "cudf_groupby"):
        ...
        if conversion.original_kind == "cudf_groupby":
            ...
            if not group_cols:
                raise ValueError(
                    "Unable to restore cudf GroupBy without grouping columns."
                )
...
def conversion_to_pandas(
    ...
    if has_polars():
        ...
        if isinstance(data, pl.dataframe.group_by.GroupBy):
            ...
            if not group_cols:
                raise ValueError(
                    "Unable to resolve group columns from polars GroupBy object."
                )
