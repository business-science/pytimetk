...
from pytimetk.utils.checks import check_dataframe_or_groupby, check_dataframe_or_groupby_polars, check_date_column, check_value_column
...
def augment_fourier_v3(
    ...
    if isinstance(data, pd.core.groupby.generic.DataFrameGroupBy):
        ...
        df = pl.from_pandas(data.apply(lambda x: x)) \
            .group_by(group_names) \
            .sort(by=[*group_names, date_column], descending=False, nulls_last=True)
    else:
...
pd.core.groupby.generic.DataFrameGroupBy.augment_fourier_v3 = augment_fourier_v3
