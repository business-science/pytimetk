...
def _augment_rsi_pandas(
    ...
    if isinstance(data, pd.DataFrame):
        ...
        for col in close_column:
            for period in periods:
                df[f"{col}_rsi_{period}"] = _calculate_rsi_pandas(
                    df[col], period=period
                )
    ...
    if isinstance(data, pd.core.groupby.generic.DataFrameGroupBy):
        ...
        for col in close_column:
            for period in periods:
                df[f"{col}_rsi_{period}"] = df.groupby(group_names, group_keys=False)[
                    col
                ].apply(_calculate_rsi_pandas, period=period)
...
def _augment_rsi_polars(
    ...
    if isinstance(data, pd.core.groupby.generic.DataFrameGroupBy):
            ...
            for col in close_column:
                for period in periods:
                    pl_df = pl_df.with_columns(
                        _calculate_rsi_polars(pl_df[col], period=period).alias(
                            f"{col}_rsi_{period}"
                        )
                    )
            return pl_df
        ...
        out_df = df.group_by(*group_names, maintain_order=True).map_groups(apply_rsi)
        ...
        for col in close_column:
            for period in periods:
                _expr = _calculate_rsi_polars(pl.col(col), period=period).alias(
                    f"{col}_rsi_{period}"
                )
                _exprs.append(_expr)
