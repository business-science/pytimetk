...
def augment_stochastic_oscillator(
    ...
    if isinstance(k_periods, int):
        ...
        raise TypeError(
            f"Invalid k_periods specification: type: {type(k_periods)}. Please use int, tuple, or list."
        )
    ...
    if engine == "pandas":
        ret = _augment_stochastic_oscillator_pandas(
            data,
            date_column,
            high_column,
            low_column,
        ...
        ret = _augment_stochastic_oscillator_polars(
            data,
            date_column,
            high_column,
            low_column,
...
def _augment_stochastic_oscillator_pandas(
    ...
    for k_period in k_periods:
        if group_names:
            ...
            lowest_low = (
                df.groupby(group_names)[low_column]
                .rolling(window=k_period, min_periods=1)
                .min()
                .reset_index(level=0, drop=True)
            )
            highest_high = (
                df.groupby(group_names)[high_column]
                .rolling(window=k_period, min_periods=1)
                .max()
                .reset_index(level=0, drop=True)
            )
            df[f"{col}_stoch_k_{k_period}"] = (
                100 * (df[col] - lowest_low) / (highest_high - lowest_low)
            )
            # Calculate %D for each d_period
            for d_period in d_periods:
                df[f"{col}_stoch_d_{k_period}_{d_period}"] = (
                    df.groupby(group_names)[f"{col}_stoch_k_{k_period}"]
...
def _augment_stochastic_oscillator_polars(
    ...
    if group_names:
        ...
        for k_period in k_periods:
            ...
            ).alias(f"{col}_stoch_k_{k_period}")
        ...
        for k_period in k_periods:
            for d_period in d_periods:
                d_expr = (
                    pl.col(f"{col}_stoch_k_{k_period}")
                    .rolling_mean(window_size=d_period)
                    .over(group_names)
                    .alias(f"{col}_stoch_d_{k_period}_{d_period}")
                )
