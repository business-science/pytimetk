...
def augment_ppo(
    ...
    if engine == "pandas":
        ret = _augment_ppo_pandas(
            data, date_column, close_column, fast_period, slow_period
        )
    elif engine == "polars":
        ret = _augment_ppo_polars(
            data, date_column, close_column, fast_period, slow_period
        )
...
def _augment_ppo_pandas(data, date_column, close_column, fast_period, slow_period):
    ...
    if isinstance(data, pd.core.groupby.generic.DataFrameGroupBy):
        ...
        df = data.groupby(group_names, group_keys=False).apply(
...
def _augment_ppo_polars(data, date_column, close_column, fast_period, slow_period):
    ...
    def calculate_ppo_single(pl_df):
        ...
        return pl_df.with_columns(
            [
                ppo_line.alias(f"{close_column}_ppo_line_{fast_period}_{slow_period}"),
            ]
        )
    ...
    if "groupby" in str(type(data)):
        result_df = (
            ...
            .map_groups(calculate_ppo_single)
            .to_pandas()
        )
