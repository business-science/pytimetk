...
When building machine learning models, we need to setup our dataframe to hold information about the future. This is the dataframe that will get passed to our ```model.predict()``` call.
This is made easy with ```tk.future_frame()```.
...
Let's create a label to split up our actuals from our predction dataset before recombining.
