...
Timetk is designed to work with any time series domain. **Arguably the most important is Finance.** This tutorial showcases how you can perform Financial Investment and Stock Analysis at scale with `timetk`. This applied tutorial covers financial analysis with:
...
Let's see a few examples of technical indicators in `timetk`. 
...
Here's how you can calculate and plot Bollinger Bands with `timetk` using **this code template (click to expand)**:
...
The problem is that the stock market is constantly changing. And these descriptive statistics aren't representative of the most recent fluctuations. This is where `timetk` comes into play with rolling descriptive statistics. 
...
The `timetk` package offers a wide range of versatile time series functions, many of which can help improve Financial, Stock, Portfolio, and Investment Analysis in Python. 
