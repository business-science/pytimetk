...
| [plot_time_series_boxplot](plot_time_series_boxplot.qmd#pytimetk.plot_time_series_boxplot) | Visualize rolling distributions of a time series by aggregating values into |
| [plot_time_series_regression](plot_time_series_regression.qmd#pytimetk.plot_time_series_regression) | Fit a linear regression using a formula and visualise observed vs fitted |

## 🚀 Plot Correlation, Seasonality, and Trends

Visualize correlation, seasonality, and trends with one line of code.

| | |
| --- | --- |
| [plot_acf_diagnostics](plot_acf_diagnostics.qmd#pytimetk.plot_acf_diagnostics) | Visualise ACF, PACF, and optional CCF diagnostics with Plotly, including |
| [plot_seasonal_diagnostics](plot_seasonal_diagnostics.qmd#pytimetk.plot_seasonal_diagnostics) | Visualize seasonal patterns using box or violin plots grouped by seasonality |
| [plot_stl_diagnostics](plot_stl_diagnostics.qmd#pytimetk.plot_stl_diagnostics) | Visualize STL decomposition components (observed, season, trend, remainder, |
| [acf_diagnostics](acf_diagnostics.qmd#pytimetk.acf_diagnostics) | Compute tidy autocorrelation, partial autocorrelation, and optional |
| [seasonal_diagnostics](seasonal_diagnostics.qmd#pytimetk.seasonal_diagnostics) | Prepare seasonal feature diagnostics akin to ``tk_seasonal_diagnostics``. |
| [stl_diagnostics](stl_diagnostics.qmd#pytimetk.stl_diagnostics) | Generate STL decomposition diagnostics (observed, season, trend, remainder, seasadj). |
...
| [resolve_lag_sequence](resolve_lag_sequence.qmd#pytimetk.resolve_lag_sequence) | Normalise lag specifications into a sorted numpy array of non-negative integers. |
| [parse_human_duration](parse_human_duration.qmd#pytimetk.parse_human_duration) | Convert human-friendly duration input into a pandas Timedelta or DateOffset. |

## 🧰 Tidy Helpers

Column selection and duration parsing utilities for diagnostics APIs.

| | |
| --- | --- |
| [contains](contains.qmd#pytimetk.contains) | Build a selector that returns columns containing ``pattern``. |
| [starts_with](starts_with.qmd#pytimetk.starts_with) | Build a selector that returns columns starting with ``prefix``. |
| [ends_with](ends_with.qmd#pytimetk.ends_with) | Build a selector that returns columns ending with ``suffix``. |
| [matches](matches.qmd#pytimetk.matches) | Build a selector that returns columns matching the supplied regular expression. |
| [resolve_column_selection](resolve_column_selection.qmd#pytimetk.resolve_column_selection) | Resolve flexible column selectors into a concrete list of column names. |
...
| [theme_plotly_timetk](theme_plotly_timetk.qmd#pytimetk.theme_plotly_timetk) | Apply pytimetk styling to any Plotly ``go.Figure``. |
