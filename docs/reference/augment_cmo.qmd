...
| `close_column`  | Union\[str, List\[str\]\]                                       | The `close_column` parameter is used to specify the column(s) in the input data that contain the values on which the CMO will be calculated. It can be either a single column name (string) or a list of column names (if you want to calculate CMO on multiple columns).                                                                            | _required_ |
