...
`augment_cmo(data, date_column, value_column, periods=14, reduce_memory=False, engine='pandas')`
...
| `value_column`  | Union\[str, List\[str\]\]                                       | The `value_column` parameter is used to specify the column(s) in the input data that contain the values on which the CMO will be calculated. It can be either a single column name (string) or a list of column names (if you want to calculate CMO on multiple columns).                                                                            | _required_ |
| `periods`       | Union\[int, Tuple\[int, int\], List\[int\]\]                    | The `periods` parameter in the `augment_cmo` function specifies the number of periods over which the Chande Momentum Oscillator (CMO) is calculated. It can be provided as an integer, a tuple of two integers (start and end periods), or a list of integers.                                                                                       | `14`       |
