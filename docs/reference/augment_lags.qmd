...
| date_column  | str or ColumnSelector                  | The column containing timestamps used to order the data before shifting. Accepts tidy selectors (e.g., ``contains("date")``) for convenience.                                                                                                                                                                                                                                                                                                                                                                                                                           | _required_ |
| value_column | str, ColumnSelector, or list           | The `value_column` parameter is the column(s) in the DataFrame that you want to add lagged values for. It can be either a single column name, a tidy selector, or a list mixing both.                                                                                                                                                                                                                                                                                                                                                                                   | _required_ |
| lags         | int, tuple, list, or str               | The `lags` parameter is an integer, tuple, or list that specifies the number of lagged values to add to the DataFrame.  - If it is an integer, the function will add that number of lagged   values for each column specified in the `value_column` parameter.  - If it is a tuple, it will generate lags from the first to the second   value (inclusive).  - If it is a list, it will generate lags based on the values in the list. - If it is a string (e.g. ``"3 days"``), the duration is converted   into the number of observations implied by ``date_column``. | `1`        |
