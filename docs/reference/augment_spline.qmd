...
Adds spline basis expansions for a numeric column. Supports both pandas and polars engines.

## Parameters {.doc-section .doc-section-parameters}

| Name              | Type                                                     | Description                                                                                                                                     | Default    |
|-------------------|----------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------|------------|
| data              | pd.DataFrame or pd.core.groupby.generic.DataFrameGroupBy / pl.DataFrame or pl.dataframe.group_by.GroupBy | Input data or grouped data.                                                                                                                     | _required_ |
...
| reduce_memory     | bool                                                     | If True, attempt to downcast numeric columns to reduce memory usage (pandas engine only).                                                       | `False`    |
| engine            | str                                                      | Execution engine. Use `'pandas'` for pandas objects or `'polars'` for polars DataFrames/GroupBy objects.                                        | `'pandas'` |

## Returns {.doc-section .doc-section-returns}

| Name   | Type                       | Description                                   |
|--------|----------------------------|-----------------------------------------------|
|        | pd.DataFrame \| pl.DataFrame | DataFrame with spline basis columns appended. |
...
df_polars_spline = tk.augment_spline(
    data=df_polars,
    column_name='step',
    spline_type='bs',
    df=5,
