...
| periods       | int, tuple, list, or str               | The `periods` parameter is an integer, tuple, or list that specifies the periods to shift values when differencing.  - If it is an integer, the function will add that number of differences   values for each column specified in the `value_column` parameter.  - If it is a tuple, it will generate differences from the first to the second   value (inclusive).  - If it is a list, it will generate differences based on the values in the list. - If it is a string (e.g. ``"7 days"``), the duration is converted into   the equivalent number of observations derived from ``date_column``. | `1`        |
