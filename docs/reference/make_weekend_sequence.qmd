...
| `engine`          | str                                 | The `engine` parameter is used to specify the engine to use for generating a weekdend series. It can be either "pandas" or "polars". - The default value is "pandas". - When "polars", the function will internally use the `polars` library for generating a weekend series. This can be faster than using "pandas" for large datasets. | `'pandas'` | 

## Returns

| Type      | Description                                      |
|-----------|--------------------------------------------------|
| pd.Series | A Series containing the generated weekend dates. |
