...
| alpha         | float or sequence of floats            | The `alpha` parameter represents the smoothing factor for the Exponential Weighted Moving (EWM) window function. It controls the rate at which the weights decrease exponentially as the data points move further away from the current point. Pass a single value to compute one EWM or a sequence of values to generate multiple EWM columns in a single call. This option is mutually exclusive with specifying decay parameters such as `com`, `span`, or `halflife` through ``**kwargs``. | `None`     |
