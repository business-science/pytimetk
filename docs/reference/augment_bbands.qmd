...
`augment_bbands(data, date_column, close_column, periods=20, num_std_dev=2, reduce_memory=False, engine='pandas')`
...
| `num_std_dev`   | float                                                           | The `num_std_dev` parameter is a float that represents the number of standard deviations to use when calculating the Bollinger Bands. Bollinger Bands are a technical analysis tool that consists of a middle band (usually a simple moving average) and an upper and lower band that are typically two standard                          | `2`        |
